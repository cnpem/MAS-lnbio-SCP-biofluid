...
def preprocess(file):
    ...
    good_matches = 'OneSeqPerGene_Hsapiens_UP000005640_9606'
    crap = data['PG.FastaFiles'] != good_matches
    non_crap = data['PG.FastaFiles'] == good_matches
    ...
    data = data[non_crap]
    data.drop(['PG.Genes', 'PG.ProteinDescriptions',
               'PG.FastaFiles'], axis=1, inplace=True)
...
# TODO fix colors visualize_high_dim_clusters according to custom palette

base_path = "/home/fabio/Desktop/output/spectronaut/figura2"
...
for file in df_globs:
    ...
    tmp_labels = ["_".join(x.split("_")[0:2]) for x in tmp_df.columns[1:]]
    tmp_labels = [sub(r"^\[\d+\]\s", "", x) for x in tmp_labels]
    df_dict[pd.Series(tmp_labels).unique()[0]] = tmp_df
df_dict.keys()
...
# blank cellenone: #C294CF
# blank 10 samples: #A6DBA0
# blank saliva: #E1AF7A
# blank tears: #96C5DE
# saliva: #D9762B
# lagrima: #3466AC
